...
def Q_metric(N_train, X_train, y_train, B, K, xi, gamma, nested_C, lambda_penalty):
    """
    Returns
    -------
    ...
    Q_QAC = np.zeros((nested_C * N_var, nested_C * N_var))

    for c_raw in range(nested_C):
        for c_col in range(nested_C):
            for i in range(N_var):
                for j in range(N_var):
                    ...
                    if i == j:
                        if c_raw == c_col:
                            Q_QAC[p, q] += nested_C * Q_upper[i, i]
                    else:
                        Q_QAC[p, q] += Q_upper[i, j]

    if lambda_penalty != 0.0:
        for i in range(N_var):
            for c in range(nested_C):
                for cp in range(c + 1, nested_C):

                    p = c  * N_var + i
                    q = cp * N_var + i

                    Q_QAC[p, p] += lambda_penalty
                    Q_QAC[q, q] += lambda_penalty

                    Q_QAC[p, q] += -lambda_penalty
                    Q_QAC[q, p] += -lambda_penalty

    Q_QAC_upper = np.zeros_like(Q_QAC)

    for p in range(nested_C * N_var):
        Q_QAC_upper[p, p] = Q_QAC[p, p]
        for q in range(p + 1, nested_C * N_var):
            Q_QAC_upper[p, q] = Q_QAC[p, q] + Q_QAC[q, p]


    return K_train_train, Q_QAC_upper

#Solver 구하기

def Quantum_Solver(Q_upper, label_name):
    label_name = str(label_name)

    qpu = DWaveSampler(solver={"name": "Advantage_system6"})
    ...
    top_solutions = sampleset.truncate(20, sorted_by="energy")

    print("top solutions:", len(top_solutions))
...
def alpha_value(N_train, x_opt, B, K, nested_C):

    x_sample = x_opt.reshape(nested_C, K*N_train)
    ones_count = np.sum(x_sample, axis=0)
    x_opt = (ones_count >= (nested_C / 2)).astype(int)
...
def Test_Graph(X_train, X_test, y_train, alpha, K_train_train, gamma, C):
